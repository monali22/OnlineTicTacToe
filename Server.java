...
public class Server {
	...
	public static void main(String[] args) {
		if (args.length == 1)
			...
			// invalid arguments
			System.err.println("usage: java -cp jsch-0.1.54.jar:. JSpace.Server config.txt");
			System.exit(-1);
		}
	}
	 ...
	 *
	 * @param configFile a configuration file name. It must include: user account
	 *                   (1st line); password (2nd line); and all remote host IP
	 *                   names (the rest).
	 */
	public Server(String configFile) {
		...
		String command = "java -cp " + cur_dir + "/jsch-0.1.54.jar:" + cur_dir + " Server";
		...
		for (int i = 0; i < hosts.size(); i++) {
			...
			// );
			connections[i] = new Connection(username, password, hosts.get(i), command);
		}
	...
	public Server() {
		...
		try {
			...
			// IMPLEMENT BY YOURSELF
			String msg = "Hello from Slave " + InetAddress.getLocalHost().getHostName();
